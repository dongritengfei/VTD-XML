...
public class XPathDemo extends JApplet {
	...
	public void init() {
		 ...
		 * nav_root.addActionListener(new navRootListener()); nav_parent= new
		 * JButton("toElement(PARENT)"); nav_parent.addActionListener(new
		 * navParentListener()); nav_first_child = new
		 * JButton("toElement(FIRST_CHILD)");
		 * nav_first_child.addActionListener(new navFCListener());
		 * nav_last_child = new JButton("toElement(LAST_CHILD)");
		 * nav_last_child.addActionListener(new navLCListener()); nav_next_sib =
		 * new JButton("toElement(NEXT_SIB)");
		 * nav_next_sib.addActionListener(new navNSListener()); nav_prev_sib =
		 * new JButton("toElement(PREV_SIB)");
		 * nav_prev_sib.addActionListener(new navPSListener()); nav_push = new
		 * JButton("push"); nav_push.addActionListener(new navPushListener());
		 * nav_pop = new JButton("pop"); nav_pop.addActionListener(new
		 * navPopListener());
		 */
		...
		/*
		 * navPanel.add(nav_root); navPanel.add(nav_parent);
		 * navPanel.add(nav_first_child); navPanel.add(nav_last_child);
		 * navPanel.add(nav_next_sib); navPanel.add(nav_prev_sib);
		 * navPanel.add(nav_push); navPanel.add(nav_pop);
		 */
		// if(!test_text){
	...
	public void setButtons(boolean b) {
		/*
		 * nav_root.setEnabled(b); nav_parent.setEnabled(b);
		 * nav_first_child.setEnabled(b); nav_last_child.setEnabled(b);
		 * nav_next_sib.setEnabled(b); nav_prev_sib.setEnabled(b);
		 * nav_push.setEnabled(b); nav_pop.setEnabled(b);
		 */
	...
	public class parse_ns_Listener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try {
				...
				/*
				 * for (int i=0;i<vn.getTokenCount();i++) { int type =
				 * vn.getTokenType(i); if (type == VTDNav.TOKEN_STARTING_TAG ||
				 * type == VTDNav.TOKEN_ATTR_NAME || type ==
				 * VTDNav.TOKEN_ATTR_NS) { System.out.println("Prefix: "
				 * +((vn.getTokenLength(i)>>16)&0xffff)); System.out.println(
				 * "Q length "+(vn.getTokenLength(i)&0xffff)); } }
				 */
				showToken();
	...
	/*
	 * public class navRootListener implements ActionListener { public void
	 * actionPerformed(ActionEvent e){ try{ status.setText("");
	 * vn.toElement(VTDNav.ROOT); showToken(); status.setText(
	 * "current index is "+vn.getCurrentIndex()); } catch (NavException ne){
