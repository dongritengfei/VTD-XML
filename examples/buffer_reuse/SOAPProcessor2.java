...
public class SOAPProcessor2 {
	...
	public static void main(String argv[]) {
		try {
			...
			// Modify this part of the code if the message is transmitted via
			// HTTP
			...
			int j = 0;
			while (j < 10) {
				vg.setDoc_BR(b); // use setDoc_BR (instead of setDoc) to turn on
									// buffer reuse
				...
				while ((t = ap.evalXPath()) != -1) {
					...
					int len = (int) (l >> 32);
					int offset = (int) l;
					fos.write(b, offset, len); // write the fragment out into
												// out.txt
					fos.write("\n=========\n".getBytes());
				}
				ap.resetXPath();
